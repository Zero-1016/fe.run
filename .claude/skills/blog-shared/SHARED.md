...
### §RULE-TRANSLATIONESE — 번역투

**규칙**: 영어 직역체 표현을 한국어다운 표현으로 바꿉니다. AI 글의 가장 흔한 시그니처예요.

**금지 표현**:

- `~를 통해` → `~로`, `~으로`
- `~에 있어서` → `~에서`, `~에`, 또는 삭제
- `~에 의해` → 능동으로 전환 (`A 가 B 했다`)
- 이중 피동: `~되어진다`, `~지어진다`, `~되어지는` → 단일 피동 (`~된다`, `~지는`) 또는 능동

**판단 기준**: 영문법 흔적이 남은 우회 표현이면 위반. 직역체가 의도된 인용·번역
컨텍스트는 예외.

**예시**:

```
❌ "useMemo 를 통해 성능을 최적화할 수 있어요."
✅ "useMemo 로 성능을 최적화할 수 있어요."

❌ "React 에 있어서 상태 관리는 중요해요."
✅ "React 에서 상태 관리는 중요해요."

❌ "이 함수는 React 에 의해 호출됩니다."
✅ "React 가 이 함수를 호출해요."
...
### §RULE-FORMAL-NOUN — 형식명사 남용

**규칙**: 문장을 늘리는 형식명사 우회 표현을 단정형으로 줄입니다.
...
- `~점이다`, `~라는 점` → 핵심 명사로 직접
- `~할 필요가 있다` → `~해야 한다`, `~해요`
- `~할 수 있을 것으로 보인다` → `~할 수 있어요`, `~할 것 같아요`

**판단 기준**: 형식명사가 의미 없이 문장 길이만 늘리면 위반. 강조나 대조 목적의
의도적 사용은 예외.

**예시**:

```
❌ "useMemo 가 성능을 개선해 주는 것이다."
✅ "useMemo 가 성능을 개선해요."

❌ "의존성 배열을 정확히 관리할 필요가 있다."
✅ "의존성 배열을 정확히 관리해야 해요."

❌ "이는 React 의 렌더링 최적화에 도움이 된다는 점이다."
✅ "이건 React 렌더링 최적화에 도움이 돼요."
```

### §RULE-NO-ESCAPE — 도망치는 마무리

**규칙**: 본문 후반이나 섹션 마무리에서 글의 약속을 줄여버리는 후퇴 표현을 쓰지 마세요.
글 범위는 도입에서 미리 선언해야지, 마무리에서 줄어들면 독자가 도망친 인상을 받습니다.

**금지 표현**:

- "한 글에서 다 풀 양이 아니다", "한 편에서 다 다루기엔"
- "범위가 너무 커서 여기서는 ~", "이건 따로 한 글이 필요하다"
- "이 글은 여기까지" 류의 갑작스런 단절
- "다음 글에서 다루겠다" (도입에서 시리즈로 선언한 경우는 예외)

**대체 방법**:

- 도입에서 범위 한정 명시: "이 글은 X 만 다룬다"
- 마무리는 글이 답한 질문을 한 줄로 다시 묶어주기
- 처방이 여럿이면 §RULE-PRESCRIPTION 에 따라 핵심 1개만 코드로 보여주고
  나머지는 도입에서 범위 외로 선언

**판단 기준**: 글 마지막 2개 단락에서 "이 글은 못 한다", "여기서는 안 다룬다" 류
표현이 나오는데 도입에 그 한정이 없으면 위반. 도입에 있으면 통과.

### §RULE-LEAD-DIRECT — 도입부 직접 답 패턴

**규칙**: 오프닝 단락(첫 H2 이전, 약 200자·3문장 이내)은 "장면/질문 + 한 문장
핵심 답"을 모두 갖춰야 합니다. 마지막 문장에 글의 한 문장 축이 명시되어야 해요.

**Why**: ChatGPT·Perplexity·Google AI Overview 같은 답변 엔진은 글의 첫
200~300자만 보고 인용 여부를 결정합니다. 첫 단락에 핵심 답이 없으면 글의 주제를
못 잡고 인용하지 않아요. 동시에 "장면을 용어 앞에 둔다"는 진입성 원칙도 유지해야
하므로, 두 요구를 **장면 → 핵심 답** 순서로 결합합니다.

**구조**:

- 첫 1~2문장: 독자가 마주하는 장면 또는 질문 (1인칭 경험 한 줄 허용)
- 마지막 1문장: 글의 한 문장 축 = 이 글이 답하는 한 문장

**금지**:

- 장면만 있고 핵심 답이 없는 경우 (답변 엔진이 주제 못 잡음)
- 핵심 답이 다섯 문장 뒤에 등장 (도입부가 너무 길어짐)
- 첫 단락이 용어 정의로 시작 (진입성 저하, §RULE-META 와도 충돌)

**위반 예시**:

> "처음 fetch 를 끊는 데 AbortController 를 썼어요. 그런데 알고 보니 다른 곳에서도
> 쓸 수 있더라구요. 이걸 정리해 보고 싶었어요."

장면만 있고 한 문장 답이 없어요. "AbortController 는 fetch 뿐 아니라 어떤 비동기
작업이든 끊을 수 있어요" 같은 축이 마지막에 와야 합니다.

**통과 예시**:

> "한국 사용자와 독일 사용자가 같은 URL 을 열어도 다른 서버가 응답할 수 있어요.
> 같은 도메인이라도 사용자마다 다른 거점이 응답한다는 게 이 글의 한 가지 축이에요."

장면 한 문장 + 한 문장 축. 200자 이내.

**판단 기준**: 오프닝 단락(첫 H2 직전)을 읽었을 때, "이 글은 X 에 답한다"의 X 가
한 문장으로 추출되면 통과. 추출이 모호하면 위반.

**§RULE-META 와의 관계**: "이 글에서는 ~를 다룹니다" 같은 메타 문장은 여전히 금지.
핵심 답은 메타 예고가 아니라 실제 결론을 한 문장으로 던지는 형태여야 해요.
...
### §RULE-RHYTHM-CONJUNCTION — 문두 접속사 반복

**규칙**: `또한`, `따라서`, `즉`, `그리고`, `하지만` 같은 문두 접속사가 3개 단락
이상 연속해서 같은 단어로 시작되면 단조롭습니다.

**판단 기준**: 인접한 3개 이상의 단락이 같은 접속사로 시작하면 위반. 2개까지는
허용. 다른 접속사 (예: 따라서 → 그리고 → 또한) 가 섞이면 통과.
...
**대체 방법**: 접속사 자체 삭제 (한국어는 접속사 없이도 흐름이 통함), 다른
접속사로 교체, 또는 문장 구조 변경 (`이런 점에서`, `그런 이유로` 같은 구체적
연결어).
...
**규칙**: 글이 "X 하지 말고 Y 써라" 같은 진단 → 처방 구조를 가질 때, 처방 측에 최소
1개의 동작하는 코드 예시가 있어야 합니다. 처방을 글로만 설명하면 독자에게 절반만
도착한 글이 됩니다.

**적용 범위**:

- 진단만 다루는 글은 해당 없음
- 처방이 여러 개면 가장 핵심 처방 1개에만 코드 의무 (전부 의무는 아님)
- 코드는 의사 코드가 아니라 동작하는 형태 — `import` 부터 호출 1줄까지 보이는 최소 단위

**예외**:

- 도입에서 범위를 명시한 경우. 예: "이 글은 cloneElement 의 타입 함정에 집중한다,
  대안 패턴의 비교는 다루지 않는다"
- 처방이 "이미 알려진 라이브러리 호출" 인 경우. `import Hls from 'hls.js'` +
  `new Hls(...)` 한 줄로 충분

**판단 기준**: 글에 "X 대신 Y 를 써라" 류 처방이 나오는데, Y 의 코드가 0줄이면 위반.
도입에서 범위 외 선언이 있거나, Y 가 단순 라이브러리 호출이면 통과.

**§RULE-NO-ESCAPE 와의 관계**: 처방이 여러 개라 1개만 코드로 보여주는 경우, 나머지를
"한 글에서 다 풀 양이 아니다" 로 처리하면 §RULE-NO-ESCAPE 위반. 도입에서 "이 글은
X 패턴만 다룬다" 로 선언하면 둘 다 통과.
...
**topic-suggest 와의 분리**: `blog-topic-suggest` 가 추천 단계에서 보여주는
영감 도메인은 글감 후보의 출처 표시 전용입니다. 본문 인라인 링크나
`<References />` 에 자동으로 들어가지 않습니다. blog-write 가 본 작업에서
1순위 자료를 새로 수집하고, External Mention/References 는 그 1순위 자료를
대상으로만 적용합니다.
...
- **번호 기반 스펙 참조**: RFC 6749, RFC 7636, ECMA-262 등 번호로 부르는
  명세. 번호만으로는 무엇인지 알 수 없으므로 첫 등장 시 한 줄 풀이 필수
...
**번호 기반 스펙 주의**: RFC 6749 같은 번호는 풀어쓰기(형식 1)가 불가능해요.
형식 2(한 줄 정의) 또는 형식 3(인라인 링크 + 풀이)으로 "그게 무슨 표준인지"를
한 줄로 밝히세요. 예: "[RFC 6749](https://datatracker.ietf.org/doc/html/rfc6749),
OAuth 2.0 의 규칙을 못 박은 공식 표준 문서예요"
...
**구조에 형태를 맞추기 (막연히 노드를 흩뿌리지 말 것)**:

표현하려는 실제 관계 구조를 먼저 보고, 거기에 다이어그램 형태와 `edgeType` 을 맞춘다.

- 요청·응답 왕복 (RFC 프로토콜 흐름처럼 "A 요청 → B 응답" 식): 한쪽 주체를 `h` 로
  키운 세로 허브 노드로 두고, 나머지 역할을 반대편에 세로로 쌓은 뒤
  `edgeType="straight"` 로 곧은 직선. 같은 두 노드 사이에 양방향 화살표를 직접 겹쳐
  그리면 짧은 카드에서 화살표·라벨이 뭉쳐 실패한다. 세로 허브로 키우면 레인 자동
  분배가 화살표를 세로로 펼쳐 RFC 도식처럼 그려진다.
- 삼각형·대각선 토폴로지: `straight` 는 선이 비스듬해져 어색하니 기본 `smoothstep`.
- 단순 분기·다층: 기본 `smoothstep` 으로 충분.

세부 동작(`h`, `edgeType`, 레인 자동 분배, 양방향)은 아래 `components/ui/CLAUDE.md`
참조를 따른다.
...
### §MDX-FAQ — FAQ 컴포넌트 사용 규칙

**사용 시점**: 개념·기법 설명 글의 마지막 절로 "자주 묻는 질문" 형태가 자연스러울 때.

**AEO 효과**: `FAQPage` schema 가 자동 주입되어 ChatGPT·Perplexity·Google AI Overview
같은 답변 엔진이 Q&A 쌍을 직접 인용할 수 있게 됩니다.

**제약**:

- **한 글에 1개만** (Google FAQPage 가이드).
- **question 은 독자 질문형**. "X 정리" 같은 제목형 X, "X 는 무엇인가요?" 같은
  질문형 O.
- **answer 는 한두 문단 직접 답**. 복잡한 코드 블록은 본문에 두고 FAQ 에는 결론만.
  schema 의 `acceptedAnswer.text` 는 텍스트만 추출되므로 JSX 안 코드는 의미 없어요.
- **글 마지막 절 (References 직전) 에 배치**.
- 강제 아님 — 선택적 사용.

**판단 기준**: 본문에 다 못 푼 후속 질문이 2개 이상 남아 있으면 FAQ 절 추가를 검토.
한두 개뿐이면 본문 안에 흡수.

**상세 props**: `components/ui/CLAUDE.md` 의 FAQ 섹션 참조 (SSOT).
...
## §META-EXPRESSION-METRICS — 표현 검토 정량 지표

expression-review 와 blog-revise 가 글의 어조 회귀를 추적하기 위해 사용하는 정량
지표 정의예요. 리포트 끝에 한 블록으로 출력해서 다듬기 전후 비교가 가능하게 해요.
...
- 어미 분포: 구어 78% / 격식 20% / 문어 2%
- 문장 길이: 평균 32자 (σ=14)
- 구조: 24단락 / 86문장 / 단락당 3.6문장
```

### 사용처

- `blog-expression-review`: 정상 모드와 dry_run 모드 리포트 끝에 출력
- `blog-revise`: 패턴 1, 2, 3 검증 사이클 후 출력
- 비교용: 사용자가 git 이력으로 이전 리포트와 대조 가능
...
## §TOPIC-SUGGEST-FLOW — 글감 추천 파이프라인 (포인터)

`blog-topic-suggest` 스킬이 따르는 파이프라인의 메타 규칙입니다. 상세 동작은
`.claude/skills/blog-topic-suggest/SKILL.md` 에 있고, 여기에는 다른 스킬과
충돌하지 않도록 지켜야 할 경계만 명시합니다.

### 라벨 정의

각 추천 후보에 정확히 하나의 라벨이 붙습니다.

- `[트렌드]` — `posted_date` 가 최근 90 일 이내. 최신 발표·릴리스·실험 글에서
  영감을 받은 후보
- `[에버그린]` — `posted_date` 가 1 년 초과 또는 URL 경로에 `guides/`,
  `basics/`, `tutorial/`, `learn/` 포함. 발행일 영향이 적은 가이드성 후보
- `[혼합]` — 위 어디에도 안 걸리는 중간대 (90 일 ~ 1 년 또는 날짜 정보 없음)

### 중복도 임계값

기존 글과의 frontmatter 비교 점수:

- `score >= 0.5` — 자동 제외 (출력에 포함 안 함)
- `0.3 <= score < 0.5` — "유사글 있음" 표기 + 가장 유사한 글 slug 노출
- `score < 0.3` — 통과

점수 가중치는 SKILL.md Phase 4 의 결정론적 공식 따름.

### 영감 출처 vs References 분리

이 분리는 `§RULE-EXTERNAL-MENTION` 의 일부입니다:

- 추천 단계의 **영감 도메인** 은 사용자에게 "어디서 영감을 받았는지" 만 표시
- blog-write 가 본 작업에서 1순위 공식 자료를 다시 수집
- **본문 External Mention 과 References 에는 영감 도메인이 자동 포함되지 않음**

이 경계를 지키면, 영감으로 본 4 순위 글이 글의 근거로 둔갑하는 사고를 방지합니다.

### 자료 가능성 추정

각 후보의 `1순위 가능성` 라벨 (`HIGH`/`MEDIUM`/`LOW`) 은 추정값입니다. 실제
자료 확보는 사용자가 후보를 골라 blog-write 호출 시 blog-research 가
처리합니다. topic-suggest 자체는 1순위 자료를 수집하지 않습니다.

### 참조하는 skill

- `blog-topic-suggest` — 전체 흐름 구현
- (옵션) 향후 `blog-write` Phase 0 에서 참조하는 형태로 통합 가능

### 카탈로그 위치

영감 소스 카탈로그는 `domains.md` 와 분리:

- `.claude/skills/blog-shared/config/topic-sources.md` — 영감 소스 카탈로그
  (`§TOPIC-SOURCE-TREND`, `§TOPIC-SOURCE-EVERGREEN`, `§TOPIC-SOURCE-KOREA`,
  `§TOPIC-SOURCE-BLACKLIST`)

같은 도메인이 `domains.md` 와 양쪽에 등장 가능 (예: web.dev). 역할이 다를 뿐
충돌 아님.
