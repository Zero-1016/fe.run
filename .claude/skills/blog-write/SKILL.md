...
## Phase 5.7: flow-review 호출 (조건부, FlowDiagram 있을 때만)

coherence-review 까지 통과한 뒤, 본문에 `<FlowDiagram` 이 있으면 다이어그램이 실제로
잘 렌더되는지 검수. **없으면 이 Phase 전체 skip** (서버도 띄우지 않음).

### 조건 가드

```bash
grep -c "<FlowDiagram" <저장된 MDX 경로>
```

0 이면 "FlowDiagram 없음 — flow-review skip" 으로 기록하고 Phase 6 진입.

### blog-flow-review SKILL.md 로드
...
skill 이 `scripts/shoot-flow.ts` 로 라이트/다크 스크린샷을 찍고 비전으로 판정. 자동
수정은 안 하고 **수정안만 구조화해 반환**.

### 결과 처리

**문제 없음**: 리포트 저장 후 Phase 6 진입.

**수정안 있음**:

- flow-review 가 반환한 수정안(h / edgeType / y 간격 / height / 노드 수)을 받음
- 각 항목을 §UI-USER-CHOICE 에 따라 `AskUserQuestion` 으로 확인
- 승인된 수정은 오케스트레이터가 해당 FlowDiagram 블록만 Edit 으로 반영
- 수정 후 flow-review 1회 재실행으로 확인 (무한 루프 방지: 재실행은 한 번만)
...
- Phase 5.7 flow-review 추가 (조건부, FlowDiagram 있을 때만) ✅
