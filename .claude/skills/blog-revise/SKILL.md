...
마지막으로 frontmatter `updated` 를 오늘 날짜로 갱신 (자세한 절차는 아래
"공통: frontmatter `updated` 자동 갱신" 참조).
...
패턴 2 의 Step P2-4 와 동일. frontmatter `updated` 갱신도 함께 진행.
...
#### Step P4-6: frontmatter `updated` 갱신

blog-write 가 새 본문을 저장한 뒤, blog-revise 가 frontmatter `updated` 를 오늘
날짜로 추가. 자세한 절차는 아래 "공통: frontmatter `updated` 자동 갱신" 참조.
...
## 공통: frontmatter `updated` 자동 갱신

패턴 2 (부분 수정), 패턴 3 (자료 보강), 패턴 4 (완전 재작성) 의 수정 적용 직후
호출. 패턴 1 (재검증만) 과 패턴 5 (분석만) 는 본문 변경이 없으므로 건너뜀.
...
2. **frontmatter 의 `updated` 라인 존재 여부 확인**:

   ```bash
   grep -E "^updated:" <파일경로> | head -1
   ```

3. **있으면 갱신** (오늘 날짜로 덮어쓰기):

   ```
   Edit(
     file_path=<파일>,
     old_string="updated: <기존 날짜>",
     new_string="updated: <TODAY>"
   )
   ```

4. **없으면 `date:` 라인 바로 다음에 삽입**:

   ```
   Edit(
     file_path=<파일>,
     old_string="date: <기존 date 라인 전체>",
     new_string="date: <기존 date 라인 전체>\nupdated: <TODAY>"
   )
   ```

5. 갱신 후 별도 검증 없음 — SHARED.md §FRONTMATTER 의 ISO date 검증은 velite 빌드
   단계가 잡음.

### 동시 변경 안전

본문 Edit 과 frontmatter 갱신은 같은 파일을 두 번 수정. **본문 Edit 먼저 →
frontmatter 갱신** 순서 유지. 백업은 둘 다 적용 전 한 번만 (Step P2-4 의 백업이
둘 다 보호).

### 패턴 4 특수

패턴 4 는 blog-write 가 파일을 새로 씀. blog-write 결과의 frontmatter 에는
`updated` 가 없으므로 (writer 가 적지 않음), Step 4 (없으면 삽입) 만 실행.
