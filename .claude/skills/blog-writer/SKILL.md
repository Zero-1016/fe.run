...
**적지 않는 필드**: `updated` — blog-revise 가 글 수정 시 자동으로 추가/갱신
(SHARED.md §FRONTMATTER "선택 필드" 참조).
...
**§RULE-LEAD-DIRECT 준수 필수** — 마지막 문장에 글의 한 문장 축이 와야 합니다.

**해야 할 것**:

- 첫 1~2문장: **독자가 마주하는 실제 장면 또는 질문**에서 시작.
  1인칭 경험 또는 관찰 한 줄 허용 ("처음엔 이게 왜 안 되나 싶었어요").
- 마지막 1문장: **이 글이 답하는 한 문장**. 이게 글의 축이고, 답변 엔진이 인용할
  핵심 답입니다.

**금지**:

- `§RULE-META`: "이번 글에서는 ~를 다뤄봅니다" 같은 메타 문장
- `§RULE-LEAD-DIRECT` 위반: 장면만 있고 핵심 답이 없는 경우
- 이론 나열로 시작
- "~의 개요", "~이란 무엇인가" 식 교과서 오프닝

**길이**: 2~3문장, 200자 내외. 너무 길면 핵심 답이 묻혀요.

**자가 체크**: 오프닝을 다 쓴 직후 한 번 확인 — "마지막 문장에서 이 글의 한 문장
축이 추출되는가?" 추출 안 되면 다시 씁니다.
...
**FAQ 절 추가 (선택, §MDX-FAQ 참조)**:

개념·기법 설명 글이고 본문에 다 풀지 못한 후속 질문이 2개 이상 남아 있으면
글 마지막 절 (References 직전) 에 `<FAQ>` 추가 권장. AEO 효과 (답변 엔진이
Q&A 쌍을 직접 인용) 가 큽니다. 강제 아님. question 은 독자 질문형, answer 는
한두 문단 직접 답. 한 글에 1개만.
...
- 도입부 직접 답 패턴 (§RULE-LEAD-DIRECT): 오프닝 마지막 문장에서
  "이 글은 X 에 답한다"의 X 가 한 문장으로 추출되는가?
- 콜론 구조
- `**` 한글 인접
- 금지 패턴 (삼단/과장/메타)
- 어미 비율 균형
