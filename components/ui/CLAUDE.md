...
자주 묻는 질문 섹션. 본문에 `<dl>` 구조로 렌더하면서 `FAQPage` JSON-LD를
자동 주입한다. 답변 엔진(ChatGPT, Perplexity 등) 인용도를 높이기 위한 AEO
컴포넌트.

```typescript
interface FAQItem {
  question: string; // 실제 독자 질문 형태 ("X는 왜 Y인가요?" 등)
  answer: React.ReactNode; // 마크다운/JSX 허용, schema에는 텍스트만 추출
}
...
<FAQ
  items={[
    {
      question: "border-box를 전역으로 적용해도 괜찮나요?",
      answer:
        "대부분의 경우 안전합니다. UA 기본값(content-box)에 의존하는 외부 위젯이 거의 없고, 레이아웃 계산이 훨씬 직관적이에요.",
    },
    {
      question: "content-box로 돌려야 할 때가 있나요?",
      answer:
        "써드파티 위젯 일부가 content-box 가정으로 동작합니다. 해당 위젯 컨테이너만 국소적으로 되돌리면 됩니다.",
    },
  ]}
/>
```
...
- 한 글에 1개만 사용 (Google FAQPage 가이드).
- `question`은 제목이 아니라 **독자 질문 형태**로. ("box-sizing 정리" X → "box-sizing은 무엇이 다른가요?" O)
- `answer`에 JSX를 넣어도 schema의 `acceptedAnswer.text`는 텍스트만 추출됨.
  복잡한 코드 블록은 본문에 두고 FAQ에는 한두 문단의 직접 답만.
- 글 마지막 절(References 직전)에 두는 게 자연스럽다.
